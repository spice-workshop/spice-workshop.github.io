...
const ScheduleEventRow: FC<Props> = ({ event, eventId }) => {
  ...
  const getContainerClasses = (color: typeof event.highlight) => {
    switch (color) {
      ...
      case 'orange':  return 'bg-orange-50 dark:bg-orange-500/20 border-orange-100 dark:border-orange-900/30 hover:border-orange-200';
  ...
  return (
    <div
      id={event.linkId}
      className={`flex flex-col p-4 rounded-lg transition-colors border border-transparent group ${getContainerClasses(event.highlight)} ${event.isCurrent ? 'border-l-4 !border-l-green-500' : ''}`}
    >
